...
bool Backup::restore(const QString &file)
{
    ...
    if (!schema(live,currentSchema,error)) return fail(error);
    {
        ...
        if (!version.exec("SELECT MAX(version) FROM schema_migrations") || !version.next() || version.value(0).toInt()!=7)
            return fail("Versão de backup não suportada. Esta versão restaura bancos na versão 7.");
        version.finish();
