...
inline bool removeAuthMigration() {
    ...
    return q.exec("ALTER TABLE cash_sessions DROP COLUMN closed_user_id") && q.exec("DROP TABLE IF EXISTS audit_log") && q.exec("DROP TABLE users") && q.exec("DELETE FROM schema_migrations WHERE version IN (6,7)");
}
