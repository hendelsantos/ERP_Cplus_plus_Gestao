...
As consultas funcionam após reiniciar o aplicativo. Registros antigos que não possuem itens ou pagamento detalhado são identificados sem inventar essas informações. O histórico aceita filtro opcional por período no formato `AAAA-MM-DD`; impressão/exportação ainda está pendente.
